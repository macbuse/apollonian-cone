...
def mk_cubes(scale_factor=40):
    ...
    cubes = [bb]
    for rot in [-np.pi/6,-np.pi/3, -np.pi/2] :
        bpy.ops.object.duplicate()
        bb = bpy.context.scene.objects.active
    ...
    cubes[1].location = [0,0,6]
    cubes[2].location = [0,0,-4]
...
peg_width = 1.5
peg.scale = [peg_width,peg_width,15]
peg.location = [0,0,-1]
...
apply_boolean(cones[4],peg, bool_type = 'UNION')
